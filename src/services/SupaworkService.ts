...
export class SupaworkService {
    ...
    private async resilientRequest(url: string, method: string = 'GET', body: any = null, headers: any = {}) {
        ...
        throw new Error("Koneksi gagal: Seluruh jalur proxy terblokir.");
    }
    ...
    public async generate(
        ...
        if (!tt) throw new Error("Gagal melewati Turnstile.");

        // Step 2: Token Challenge
        onProgress("Menukarkan Token Keamanan", 2);
        ...
        // Step 3: Akun Virtual
        onProgress("Membuat Identitas Virtual", 3);
        ...
        let code = "";
        for (let i = 0; i < 15; i++) {
            onProgress(`Menunggu OTP dari ${email} (${i+1}/15)`, 4);
        ...
        if (!code) throw new Error("Verifikasi email gagal (Timeout).");

        // Step 5: Login & Session
        onProgress("Autentikasi Sesi Baru", 5);
        ...
        // Step 6: Upload Image
        onProgress("Mengunggah Gambar Referensi", 6);
        ...
        // Step 7: Trigger Generator
        onProgress("Memulai AI Nano Banana", 7);
        ...
        throw new Error("Waktu render habis.");
    }
}